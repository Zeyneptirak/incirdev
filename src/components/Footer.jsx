import React from 'react'
import logo from '../assets/logoFooter.svg'
export default function Footer() {
  return (
    <div className='h-screen flex flex-col justify-center items-center'>
       
        <div className='flex flex-col md:flex-row gap-y-2 items-center justify-center gap-x-3'>
            <img src={logo} alt="" />
            <p className='text-content-grey text-[16px] font-[300] leading-8'>Figa Studio.</p>
            <p className='text-content-grey text-[16px] font-[300] leading-8'>© 2022 - Tüm hakları saklıdır. </p>
            <a className="text-white text-[16px]" href="mailto:figa@figastudio.com">
  figa@figastudio.com
</a>
            {/* <a className="text-white underline text-[16px]" href="">Çerez Politikası</a> */}
            <a className="text-white underline text-[16px]" href="/cerez-politikasi">
  Çerez Politikası
</a>

        </div>
    </div>
  )
}
